...
int main(int argc, char** argv)
{
   ...
   // printf("Thread create returned %x\n", ret);
    ...
    camBuf = (u8*)malloc(WIDTH * HEIGHT * 2 * 2);               // the buffer for the camera
    frameBuf = (u8*)malloc(WIDTH * HEIGHT * 3);  // the necessary space for the rgb 24 datas of the image
    ...
    while (aptMainLoop())
    {
        ...
        Y2RU_SetSendingYUYV((u8 *) & camBuf[0], WIDTH * HEIGHT * 2, WIDTH * 2, 0);
        Y2RU_SetReceiving((void *) &frameBuf[0], WIDTH * HEIGHT * 3, WIDTH * 3, 0);
